...
try {
    if (
        ...
        isset($_POST['shortPhase']) &&
        isset($_POST['story']) &&
        isset($_POST['reviewIsFeatured']) &&
        isset($_POST['userId']) &&
        isset($_FILES['reviewImage'])
    ) {
        ...
        if (move_uploaded_file($tempFilePath, $destinationPath)) {
            ...
            $sql = "INSERT INTO adopter_review  VALUES (NULL,?,?,?,?,CAST(? AS UNSIGNED),1,?,?,?,?)";
            ...
            if ($result === TRUE) {
                ...
                echo json_encode(
                    array(
                        "status" => "OK",
                        "code" => 200,
                        "message" => "Success adding new review.",
                    )
                );
            } else {
